...
from cspace.network.sslutil import makeServerSSLContext
...
logger = logging.getLogger('cspace.network.listener')
...
def _readServiceName( sock, sslContext, reactor, callback=None ) :
    ...
    def onInput( data ) :
        ...
        obj.stream.shutdown()
        obj.stream = None
        op.notify( (obj.sslConn,obj.peerKey,obj.peerName,obj.peerKeyID,serviceName) )
    def onHandshake( data ) :
        obj.handshakeOp = None
        if data is None :
            onError()
            return
        obj.sslConn,obj.peerKey,obj.peerName,obj.peerKeyID = data
...
class UserListener( object ) :
    ...
    def __init__( self, userName, keyId, rsaKey, updateLevelStore,
        ...
        self.sslContext = makeServerSSLContext( userName, keyId, rsaKey, self.clientCertVerifyCallback )
    ...
    def clientCertVerifyCallback(self, conn, cert, errno, depth, preverify_ok):
        return preverify_ok        
    ...
    def _onIncomingTCP( self, sock ) :
        ...
        def onRead( data ) :
            ...
            if data is None :
                return
            sslConn,peerKey,peerName,peerKeyID,serviceName = data
            if self.incomingCallback is None :
                sslAbort( sslConn )
                return
            self.incomingCallback( sslConn, peerKey, peerName, peerKeyID, serviceName )
