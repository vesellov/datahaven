...
logger = logging.getLogger('cspace.nitro.tcpbridge')
...
class TCPBridge(object) :
    def __init__( self, sock1, sock2, reactor ) :
        ...
        self.id = '%s-%s' % (str(sock1.getpeername()), str(sock2.getpeername()))
        logger.info('new bridge %s' % self.id)
    ...
    def close( self ) :
        logger.info('close bridge %s' % self.id)
    ...
    def closeInternal( self ) :
        logger.info('bridge %s closing internal' % self.id)
        for stream in self.streams :
